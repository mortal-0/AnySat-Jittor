...
class TS2CDataset(Dataset):
    ...
    #2026/5/2修改，每隔1000个样本打印一次、在开始和结束时各打印一次汇总信息
    def compute_norm_vals(self, folder, sat):
        means = []
        stds = []

        total = len(self.files)
        print(f"[compute_norm_vals] start | sat={sat} | total_files={total}")

        for i, b in enumerate(self.files):
            if i % 1000 == 0:
                print(f"[compute_norm_vals] progress: {i}/{total}")
        ...
        out_path = os.path.join(folder, "NORM_{}_patch.json".format(sat))
        with open(out_path, "w") as file:
            file.write(json.dumps(norm_vals, indent=4))

        print(f"[compute_norm_vals] done | sat={sat} | saved_to={out_path}")
    ...
    def __getitem__(self, i):
        ...
        #2025/5/2/12.04修改，解决系统路径问题，可以让代码在windows系统上运行
        file_name = self.files[i]

        class_dir = os.path.dirname(file_name)  # ...\tile\class
        tile_dir = os.path.dirname(class_dir)  # ...\tile
        dates_path = os.path.join(tile_dir, "dates.csv")

        dates = day_number_in_year(
            pd.read_csv(dates_path)["acquisition_date"].values
        )
        bands = pd.read_csv(file_name)

        label = int(os.path.basename(class_dir))
        flag = bands["Flag"].values
        mask = torch.from_numpy(flag == 0).unsqueeze(-1).repeat(1, 10)
        mask[:, 7] = False
