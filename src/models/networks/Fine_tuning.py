...
class Fine(nn.Module):
    ...
    def __init__(self, 
        ...
        # 5.13修改，在加载权重那里，把 load_state_dict 的返回值打印出来。
        if not (proj_only) and not (freeze_new) and proj_size == 1:
            msg = encoder.load_state_dict(d)
        else:
            msg = encoder.load_state_dict(d, strict=False)

        print("[DEBUG] pretrained load result:", msg)
    ...
    # 5.13修改，重写train方法，保持backbone稳定
    def train(self, mode: bool = True):
        super().train(mode)
        if self.freeze:
            self.model.eval()  # backbone 永远 eval
            if hasattr(self, "head"):
                self.head.train(mode)  # head 继续 train
        return self
